"""Restore framework error scoring (DeepWiki gap fix).

Scores framework-error: 0 (BOOLEAN) when RepairableException fires.
"""
...
class LangfuseErrorFormat(Extension):

    async def execute(self, msg: str = "", **kwargs):
        try:
            lf = _lib("langfuse_client")
            lf.score_framework_error(msg or "")
        except Exception as e:
            logger.debug(f"Langfuse error format scoring failed: {e}")

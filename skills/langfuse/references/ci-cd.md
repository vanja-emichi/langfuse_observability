...
- [ ] Confirm dataset existence and shape of the dataset items before writing code with the Langfuse CLI (run via `code_execution_tool` with `runtime: terminal`, see `references/cli.md`):
...
- [ ] If available, use the `gh` CLI via `code_execution_tool` (terminal) to check secret existence / set secrets for:
...
| Issue | Solution |
|-------|----------|
| `gh` is missing or not authenticated | Install the GitHub CLI if needed, then run `gh auth status` and `gh auth login` before using `gh secret` or `gh workflow` commands (via `code_execution_tool` terminal). |

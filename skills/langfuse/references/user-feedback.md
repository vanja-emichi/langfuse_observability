...
**For implicit feedback (server-side):** Use `langfuse.create_score()` / `langfuse.score.create()` wherever the event is already handled in application code. Fetch SDK docs for current API via `code_execution_tool` (terminal): https://langfuse.com/docs/evaluation/evaluation-methods/scores-via-sdk

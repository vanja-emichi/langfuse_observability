...
> **Note:** The a0_langfuse plugin already uses SDK v4. This guide is for upgrading OTHER applications that use older Langfuse SDKs.
...
Fetch the relevant page as markdown before implementing any changes (run via `code_execution_tool` with `runtime: terminal`):

...
Verify credentials are set before starting. Run these checks via `code_execution_tool` with `runtime: terminal` — check existence only, never print the secret key, since the value would land in the agent's context and transcripts:
...
For external templating details, fetch via `code_execution_tool` (terminal): https://langfuse.com/faq/all/using-external-templating-libraries
...
For full API, fetch via `code_execution_tool` (terminal): https://langfuse.com/docs/prompts/get-started
...
For SDK examples (Python/JS/TS), fetch via `code_execution_tool` (terminal): https://langfuse.com/docs/prompts/get-started
...
For tracing details, fetch via `code_execution_tool` (terminal): https://langfuse.com/docs/prompts/get-started#link-with-langfuse-tracing

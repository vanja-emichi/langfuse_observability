...
Credentials are **automatically loaded from the Langfuse Observability plugin config**.
Always run this before any CLI call:
...
If the config is missing or incomplete, set the variables manually (found in Langfuse UI → Settings → API Keys):
